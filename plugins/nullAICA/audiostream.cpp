...
void InitAudBuffers(u32 buff_samples)
{
	...
	RingBufferSampleCount=(aica_settings.BufferCount+2)*BufferSampleCount;
...
bool asRingStretchRead(u8* dst,u32 sz)
{
	...
	if (aica_settings.LimitFPS && used>=sz)
	{
...
void WriteSample(s16 r, s16 l)
{
	...
	while (aica_settings.LimitFPS==1 && gen_samples>128 && asRingUsedCount()>BufferSampleCount && QueryPerformanceCounter(&time_now) && (time_now-time_last)<=time_diff )
	{
		__noop;
	}

	if (aica_settings.LimitFPS==1 && gen_samples>128)
	{
		gen_samples=0;
...
void InitAudio()
{
	...
	InitAudBuffers(aica_settings.BufferSize);
