...
int showhelp(char** arg,int cl)
{
	if (cl>=1)
	{
		...
		else if (strcasecmp(arg[1],"configfile")==0)
		{
...
bool ParseCommandLine(int argc,char* argv[])
{
	...
	while(cl>=0)
	{
		...
		else if (strcasecmp(*arg,"-configfile")==0)
		{
			int as=setconfigfile(arg,cl);
			cl-=as;
			arg+=as;
		}
/*gli		else if (strcasecmp(*arg,"-slave")==0)
		{
			int as=slave_cmdl(arg,cl);
			cl-=as;
			arg+=as;
		}
		else if (strcasecmp(*arg,"-serial")==0)
		{
			int as=serial_cmdl(arg,cl);
			cl-=as;
			arg+=as;
		}*/
		else if (strcasecmp(*arg,"-about")==0)
		{
