...
struct MemChip
{
	...
	bool Load(char* root,char* prefix,const char* names_ro,const char* title)
	{
		...
		do
		{
			...
			if (curr[0]=='%')
			{
				snprintf(temp,512,"%s%s%s",base,prefix,curr+1);
			}
			else
			{
				snprintf(temp,512,"%s%s",base,curr);
			}
...
bool LoadRomFiles(char* root)
{
	...
	if (!sys_nvmem.Load(root,ROM_PREFIX,"%nvmem.bin;%flash_wb.bin;%flash.bin;%flash.bin.bin","nvram"))
	{
		if (NVR_OPTIONAL)
		{
			dlog("flash/nvmem is missing, will create new file ..");
		}
		else
		{
...
T __fastcall ReadMem_area0(u32 addr)
{
	...
	else if ((base_start >=0x005F) && (base_end <=0x005F) && (addr>= 0x005F7000) && (addr<= 0x005F70FF)) //	:GD-ROM
	{
		...
		u32 data = ReadMem_naomi(addr,sz);
#else
...
//    printf("WriteMem_area0 %08x %08x %d\n",addr,data,sz);
