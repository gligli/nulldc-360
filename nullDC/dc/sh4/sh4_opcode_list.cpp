...
//				char old=tx1[ti];
				//tx1[ti]='\0';
				dlog("Sh4Dissasm : Tag not known\"%s\"\n",tx1);
				//tx1[ti]=old;
...
//
sh4_opcodelistentry opcodes[214]=
{
...
void BuildOpcodeTables()
{
	...
	for (int i=0;i<0x10000;i++)
	{
		...
		for (int i2=0;opcodes[i2].oph;i2++)
		{
			if ((i&opcodes[i2].mask)==opcodes[i2].rez)
			{
				...
				{
					dlog("OPCODE TABLE FAULT , DOUBLE DEFINED OPCODE\n");
				}
			}
		}
	}
	...
	dlog("shil generation status : %d%% cpu done[%d of %d] , %d%% fpu done[%d of %d]\n",
