...
void DoMapleDma()
{
	...
	while (last != true)
	{
		...
		if (maple_op==0)
		{
			...
			{
				dlog("MAPLE ERROR : DESTINATION NOT ON SH4 RAM 0x%X\n",header_2);
				header_2&=0xFFFFFF;
				header_2|=(3<<26);
			...
			if (MapleDevices[device].connected && (subport==5 || MapleDevices[device].subdevices[subport].connected))
			{
				...
				#if debug_maple
					dlog("Maple :port%d_%d : 0x%02X -> done 0x%02X \n",device,subport,command,resp);
				#endif
			...
			{
				#if debug_maple
					dlog("Maple :port%d_%d : 0x%02X -> missing\n",device,subport,command);
				#endif
...
void maple_Init()
{
	sb_regs[(u32)(SB_MDST_addr-SB_BASE)>>2].flags=REG_32BIT_READWRITE | REG_READ_DATA;
	sb_regs[(u32)(SB_MDST_addr-SB_BASE)>>2].writeFunction=maple_SB_MDST_Write;

	sb_regs[(u32)(SB_MSHTCL_addr-SB_BASE)>>2].flags=REG_32BIT_READWRITE;
	sb_regs[(u32)(SB_MSHTCL_addr-SB_BASE)>>2].writeFunction=maple_SB_MSHTCL_Write;
}
