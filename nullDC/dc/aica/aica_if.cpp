...
//    printf("WriteMem_aica_reg %08x %08x %d\n",addr,data,sz);
...
void aica_sb_Init()
{
	//NRM
	//6
	sb_regs[(u32)((SB_ADST_addr-SB_BASE)>>2)].flags=REG_32BIT_READWRITE | REG_READ_DATA;
	sb_regs[(u32)((SB_ADST_addr-SB_BASE)>>2)].writeFunction=Write_SB_ADST;

	//I realy need to implement G2 dma (and rest dmas actualy) properly
	//THIS IS NOT AICA, its G2-EXT (BBA)
	sb_regs[(u32)((SB_E1ST_addr-SB_BASE)>>2)].flags=REG_32BIT_READWRITE | REG_READ_DATA;
	sb_regs[(u32)((SB_E1ST_addr-SB_BASE)>>2)].writeFunction=Write_SB_E1ST;
}
