...
static int MenuGame() {
	...
	GuiText titleTxt((char *) ROMFilename, 22, (GXColor) {
		255, 255, 255, 255
	});
	titleTxt.SetAlignment(ALIGN_LEFT, ALIGN_TOP);
	titleTxt.SetPosition(50, 50);
...
static int MenuGameSaves(int action) {
	...
	sprintf(browser.dir, "%s%s", SaveDevice, SaveFolder);
	...
	for (i = 0; i < browser.numEntries; i++) {
		...
		if (n >= 0) {
			...
			if (saves.type[j] == FILE_SNAPSHOT) {
				sprintf(scrfile, "%s%s/%s.png", SaveDevice, SaveFolder, tmp);

				memset(savebuffer, 0, SAVEBUFFERSIZE);
				if (LoadFile(scrfile, SILENT))
					saves.previewImg[j] = new GuiImageData(savebuffer, 64, 48);
			}
			snprintf(filepath, 1024, "%s%s/%s", SaveDevice, SaveFolder, saves.filename[j]);
...
//	float currentZoomVert = GCSettings.zoomVert;
	...
	float currentZoomHor = 1;
	float currentZoomVert = 1;
...
//		GCSettings.zoomVert = currentZoomVert;
		GCSettings.zoomHor = 1;
		GCSettings.zoomVert = 1;
	}
...
//				sprintf(options.value[1], "Default");
			//#ifdef HW_RVL
			sprintf(options.value[2], "HMMMMMMM");
			//#endif
...
MainMenu(int menu) {
	...
	XenosSurface * video_surface = get_video_surface();
	gameScreenImg = new GuiImage(video_surface, video_surface->width, video_surface->height);
